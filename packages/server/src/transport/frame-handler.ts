 ...
 * @see {F11-AC4} — Malformed frame → WS close 1003
 */
export function parseFrame(raw: string | unknown): FrameEnvelope | null {
	let parsed: unknown;
	...
	let causes: bigint[] | undefined = undefined;
	if (obj.causes !== undefined) {
		...
		try {
			causes = obj.causes.map((c) => {
				...
				if (typeof c === 'string') return BigInt(c);
				if (typeof c === 'number') return BigInt(Math.floor(c));
 ...
 */
export function serializeFrame(frame: FrameEnvelope): string {
