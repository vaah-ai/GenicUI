...
describe('F14-AC4: patch op with invalid path -> -32004', () => {
  describe('validatePatchPath', () => {
    it('always accepts add operations', () => {
      const op: JsonPatchOperation = { op: 'add', path: '/newField', value: 'hello' };
    ...
    it('accepts replace on existing path', () => {
      const op: JsonPatchOperation = { op: 'replace', path: '/name', value: 'new' };
    ...
    it('rejects replace on non-existent path', () => {
      const op: JsonPatchOperation = { op: 'replace', path: '/missing', value: 'new' };
      const state = { name: 'old' };
    ...
    it('accepts remove on existing path', () => {
      const op: JsonPatchOperation = { op: 'remove', path: '/name' };
      const state = { name: 'old' };
    ...
    it('rejects remove on non-existent path', () => {
      const op: JsonPatchOperation = { op: 'remove', path: '/missing' };
      const state = { name: 'old' };
    ...
    it('accepts test on existing path', () => {
      const op: JsonPatchOperation = { op: 'test', path: '/name', value: 'old' };
      const state = { name: 'old' };
    ...
    it('rejects test on non-existent path', () => {
      const op: JsonPatchOperation = { op: 'test', path: '/missing', value: 'old' };
      const state = { name: 'old' };
    ...
    it('accepts move with valid from path', () => {
      const op: JsonPatchOperation = { op: 'move', from: '/name', path: '/renamed' };
      const state = { name: 'old' };
    ...
    it('rejects move with invalid from path', () => {
      const op: JsonPatchOperation = { op: 'move', from: '/missing', path: '/target' };
      const state = { name: 'old' };
    ...
    it('accepts copy with valid from path', () => {
      const op: JsonPatchOperation = { op: 'copy', from: '/name', path: '/duplicate' };
      const state = { name: 'old' };
    ...
    it('rejects copy with invalid from path', () => {
      const op: JsonPatchOperation = { op: 'copy', from: '/missing', path: '/target' };
      const state = { name: 'old' };
    ...
    it('handles nested paths', () => {
      const op: JsonPatchOperation = { op: 'replace', path: '/props/rows/0/name', value: 'new' };
    ...
    it('accepts root path', () => {
      const op: JsonPatchOperation = { op: 'add', path: '/', value: {} };
      const state = {};
    ...
    it('handles JSON Pointer escape sequences (~1 -> ~, ~0 -> /)', () => {
      ...
      const op1: JsonPatchOperation = { op: 'replace', path: '/tilde~1', value: 'new' };
      expect(validatePatchPath(op1, state).valid).toBe(true);

      const op2: JsonPatchOperation = { op: 'replace', path: '/slash~0', value: 'new' };
