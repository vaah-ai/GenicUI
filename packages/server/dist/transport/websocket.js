 ...
 */
function hasGenicuiSubprotocol(ctx) {
    const request = ctx.request;
    const protocol = request.headers.get('sec-websocket-protocol');
    if (!protocol)
        return false;
 ...
 */
function createServerHelloFrame(sessionId) {
    const payload = {
        sessionId,
    ...
    const ping = () => {
        ...
        session.pongTimeout = setTimeout(() => {
            ...
            if (session.missedPongs >= MAX_MISSED_PONGS) {
                ...
                console.error(`[F10] Session ${session.sessionId} closed: ${session.missedPongs} missed pongs`);
