...
import { AuthError, hashApiKey, validateApiKeyFormat, parseBearerKey, parseApiKeyInfo, validateKey, scrubApiKey, safeLogKey, } from './index.js';
// --- Test helper: generate a valid API key ---
/** Generate a deterministic valid API key for testing */
function makeTestKey(prefix = 'live') {
    return `gnc_${prefix}_${'a'.repeat(32)}`;
}
/** Generate a valid random-ish API key */
function makeRandomKey(prefix = 'live') {
...
describe('F46-AC1: Format check', () => {
    ...
    it('rejects wrong prefix', () => {
        expect(validateApiKeyFormat('gnc_dev_0123456789abcdef0123456789abcdef')).toBe(false);
    });
    it('rejects short key (31 chars)', () => {
        expect(validateApiKeyFormat('gnc_live_0123456789abcdef0123456789abcde')).toBe(false);
    });
    it('rejects long key (33 chars)', () => {
        expect(validateApiKeyFormat('gnc_live_0123456789abcdef0123456789abcdeff')).toBe(false);
    });
    it('rejects empty string', () => {
        expect(validateApiKeyFormat('')).toBe(false);
    });
    it('rejects non-hex characters', () => {
        expect(validateApiKeyFormat('gnc_live_zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz')).toBe(false);
    });
});
...
describe('parseBearerKey', () => {
    ...
    it('returns null for non-Bearer prefix', () => {
        expect(parseBearerKey('Token gnc_live_0123456789abcdef0123456789abcdef')).toBe(null);
    });
