 ...
 */
function hasUnsafeKeys(v: unknown): boolean {
  if (v === null || typeof v !== 'object') return false;
  if (Array.isArray(v)) return v.some(hasUnsafeKeys);
  for (const key of Object.keys(v)) {
    ...
    if (typeof val === 'number' && Object.is(val, -0)) return true;
...
describe('JsonPatchEngine property tests', () => {
  ...
  it('F4-AC3: apply(diff(a, b), a) === b for 10K random pairs', () => {
    ...
    fc.assert(
      fc.property(safeJson(), safeJson(), (a, b) => {
