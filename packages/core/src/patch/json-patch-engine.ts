...
export class JsonPatchEngine {
   ...
   */
  public diff(before: unknown, after: unknown): Operation[] {
    ...
    if (beforeIsObj && afterIsObj) {
      ...
      if (beforeIsArray !== afterIsArray) {
        return [{ op: 'replace', path: '', value: after as unknown }];
      }
    ...
    return [
      { op: 'replace', path: '', value: after as unknown },
    ];
  }
   ...
   */
  public apply(patch: Operation[], target: unknown): unknown {
    ...
    const patched = finalResult.newDocument as Record<string, unknown>;
