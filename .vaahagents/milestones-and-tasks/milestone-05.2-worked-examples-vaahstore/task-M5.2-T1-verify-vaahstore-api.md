...
- **Post-merge architectural correction (2026-09-17).** Per user redirect, the VaahStore provider no longer lives at `packages/server/src/chat/providers/vaahstore.ts`. It moves to `examples/playground-ecommerce/server/providers/vaahstore/` (M5.2-T2-1). The 12 §5 tools' URLs, parameter shapes, and fixture-mode semantics documented in this verification report are **unchanged through the move** — only the file location and the workspace isolation rule changed. The verification report at `examples/playground-ecommerce/docs/vaahstore-api-verification.md` remains the canonical source for the 12 tools' wire shapes; M5.2-T2-1's docs update cross-references it.