...
- `examples/docs/.navigation.yml` (if it exists) — current IA tree
- `examples/docs/package.json` — installed doc-site deps

**Invoke `filesystem` MCP server** to confirm `examples/docs/` exists.
...
**Gate:** IF `examples/docs/` is missing AND `{{TASK_ID}}` is not `M5.1-T1`, THEN the docs site was never scaffolded — STOP and instruct the user to run the `prompt-task-execution.md` orchestrator with `{{TASK_ID}} = M5.1-T1` first.
